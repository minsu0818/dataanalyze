...
그렇기 때문에, 매년 수백만에서 수천만 명의 관광객이 방문하는 런던은 호텔이나 모텔과 같은 숙박업이 많이 발달한 도시이다. 숙박업이 많이 발달한 도시인 만큼 호텔도 많고, 그에 따라 리뷰 또한 많다. 리뷰의 수가 많아지면 데이터의 표본수가 증가하므로 더 정확한 모델을 설계할 수 있을 것이라고 생각했다. 그래서 나는 MobileBERT를 이용하여 런던의 호텔 리뷰에 대한 긍정 및 부정 예측을 시도해보고자 한다.   <br/><br/>
