...
## 5. 결론 및 배운 점<br/>
데이터 자체의 특수성때문에 모델의 정확도를 학습하는 단계에서 조차 난항을 겪었다. 다른 데이터들과 달리 긍 부정 평가가 따로 나눠어져 있어 하나로 합쳤는데 그래서 생긴 영향인지 결과값이 0.5가 나올때도 생기고 0.95까지 나올때도 있었다. 데이터 전처리의 중요성을 다시 한번 실감하게 된 대목이라 할 수 있었다. 또 학습 데이터의 비율이 학습부터 검증까지 많은 영향을 미친 다는것을 깨달았다. 첫번째 모델의 긍 부정의 비율을 1:1로 잡고 들어가 그런지 모델의 학습단계 부터 안정적인 Validation accuracy를 보여준 반면 불균형한 데이터의 비율로 학습을 시킨 두번째 모델은 Validation accuracy가 처음에 낮았지만 마지막에 좋아지는 상향곡선을 그려 다소 삐걱거리는 모습을 보여줬다. 마지막 검증 단계에서 조차 첫번째이 두번째 모델보다 좋은 수치를 보여줬다.  
